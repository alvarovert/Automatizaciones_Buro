...
def ejecutar_proceso_asistencia():
    """
    Ejecuta el proceso completo de asistencia y muestra los errores si los hay.
    """
    try:
        print("Iniciando descarga de Asistencia...")
        
        # Ejecutar descarga (AÑADIMOS encoding="utf-8")
        resultado_descarga = subprocess.run(
            [sys.executable, "-X", "utf8", "descarga_Asistencia_CSF.py"], 
            capture_output=True, 
            text=True,
            encoding="utf-8",
            errors="replace"
        ...
        print("✅ Se subió la Asistencia de hoy a la Base de Datos")
        print("🚀 Se completó el Proceso de Asistencia de Santander, listo para enviar.")
